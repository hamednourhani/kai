...
Receives from **Kai** (the report may have originated from `@research` or `@fact-check`, but Kai always invokes `@executive-summarizer` directly — those agents never call this one themselves, since Gemini CLI subagents cannot invoke other subagents):
...
Accepts Markdown research reports containing the above.
...
- ❌ Execute code or fetch external content (no `web_fetch` or `google_web_search` tools)
...
| Agent | Data | Trigger |
|-------|------|---------|
| Kai (report originally from `@research`) | Full research report | Summarization request |
| Kai (report originally from `@fact-check`) | Verdict report | Summary request |
...
|-----------|-----|--------|
| Original report unclear | Kai (may invoke `@research`) | Need more info |
| Claim unclear | Kai (may invoke `@fact-check`) | Need verification |
...
- Research complete (via `@research`)
...
If `@executive-summarizer` has issues:
