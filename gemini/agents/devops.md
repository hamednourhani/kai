...
- ❌ Invoke `@developer`, `@reviewer`, `@tester`, `@docs`, `@architect`, or any other subagent itself — only Kai can chain subagent calls on Gemini CLI
...
| Kai (relaying `@reviewer` output) | Code approved | Ready for deployment |
| Kai (relaying `@tester` output) | Tests passed | Ready for deployment |
| Kai (relaying `@docs` output) | Documentation complete | Ready for deployment |
...
|-----------|-----|--------|
| Infrastructure issues | External (via Kai) | Cloud provider issues |
| Build failures | Kai (may re-invoke `@developer`) | Code issues |
...
- Build failures escalate to Kai, which may re-invoke `@developer`
- Kai re-invokes `@devops` after fixes
