...
> **Note on `.kai/`:** This agent reads and writes to `.kai/postmortems/` and `.kai/memory.yaml`. `.kai/` is Kai's deliberate cross-platform project-memory convention — identical in structure and purpose across OpenCode, Claude Code, and Gemini CLI. These paths are intentional, not a porting artifact; keep them as-is.
...
`@postmortem` is NOT invoked for clean pipelines (first-pass success). As with all subagents on Gemini CLI, `@postmortem` never invokes other subagents itself — it only reports findings back to Kai, which decides what (if anything) to invoke next.
...
PREVENTION_RULE:
  ...
  examples:
    - trigger: "Python project detected but no venv/pyproject.toml"
      action: "Verify Python environment before invoking @developer"
    - trigger: "Test failures with 'connection refused' errors"
      action: "Check if required services (DB, Redis) are running before invoking @tester"
    - trigger: "@architect produced design requiring package X but package X is deprecated"
      action: "Kai should invoke @dependency-manager for a compatibility check before invoking @developer"
```
...
KAI_PRE_FLIGHT_CHECK:
  ...
  examples:
    - Rule says "verify Python env before invoking @developer"
    ...
    - Rule says "API endpoint X requires auth token in env"
      → Kai checks .env for required variables before invoking @developer
```
...
| Agent | Data | Trigger |
|-------|------|---------|
| Kai | Pipeline audit trail, failure context | Auto-trigger conditions met |

### Provides To

| Agent | Data | Format |
|-------|------|--------|
| Kai | Postmortem report, prevention rules | Markdown file + structured summary |

### Escalates To

| Condition | Via | Reason |
|-----------|-----|--------|
| Root cause requires design change | Kai (may invoke `@architect`) | Architectural decision needed |
| Recurring systemic pattern | Kai | Process change recommendation |
...
- Compiles the pipeline audit trail
- Gathers error messages and retry history
...
- Agent invocation order and outcomes
...
- Postmortem file written to `.kai/postmortems/`
- Prevention rules to index into `.kai/memory.yaml`
- Recurrence/systemic-pattern flag

### On Failure

If `@postmortem` has issues:
